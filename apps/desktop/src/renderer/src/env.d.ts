...
interface Window {
  mediaParser: {
    ...
    selectAudioFiles: () => Promise<{ files: { path: string; name: string; size: number }[]; errors: string[] }>
    audioFilesFromDrop: (files: File[]) => Promise<{ files: { path: string; name: string; size: number }[]; errors: string[] }>
